    ...
    case "$level" in
        ...
        "INFO")
            ...
            if [[ "$include_tee" == "no" ]]; then
                ...
                echo "[$timestamp] [${level_text}] $message" | tee -a "$APP_LOG_FILE"
            fi
            ;;
        "WARNING")
            level_color="$YELLOW"
            ...
            if [[ "$include_tee" == "no" ]]; then
                ...
                echo -e "[$timestamp] [${level_color}${level_text}${NC}] $message" | tee -a "$APP_LOG_FILE" 1>&2
            fi
            ;;
        "ERROR")
            level_color="$RED"
        ...
        "OK")
            ...
            if [[ "$include_tee" == "no" ]]; then
                ...
                echo -e "[$timestamp] [${level_color}${level_text}${NC}] $message" | tee -a "$APP_LOG_FILE"
            fi
            ;;
        "FAILED")
            level_color="$RED"
...
util_clean_up_log_files() {
    local log_files_list=($(find "$APP_LOG_DIR" -name "*.log" -type f -printf '%T@ %p\n' 2>/dev/null | sort -nr | cut -d' ' -f2-))
    local log_files_count=${#log_files_list[@]}

    if [[ $log_files_count -gt $APP_LOG_FILE_COUNT ]]; then
        for ((i = APP_LOG_FILE_COUNT; i < log_files_count; i++)); do
